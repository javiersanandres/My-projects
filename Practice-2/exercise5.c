...
int main(int argc, char** argv)
{
  ...
  /* check command line */
  for(i = 1; i < argc ; i++) {
    if (strcmp(argv[i], "-num_min") == 0 && (i==1)) {
      num_min = atoi(argv[++i]);
    } else if (strcmp(argv[i], "-num_max") == 0 && (i==3)) {
      num_max = atoi(argv[++i]);
    } else if (strcmp(argv[i], "-incr") == 0 && (i==5)) {
      incr = atoi(argv[++i]);
    } else if (strcmp(argv[i], "-numP") == 0 && (i==7)) {
      n_perms = atoi(argv[++i]);
    } else if (strcmp(argv[i], "-outputFile") == 0 && (i==9)) {
  ...
  /* compute times */
  ret = generate_sorting_times(QuickSort_median, nombre, num_min, num_max,incr, n_perms);
